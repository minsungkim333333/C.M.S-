def cms(Material_cost, labor_costs, kind):
    dsy = Material_cost+labor_costs
    if dsy < 500000000 and kind == '일반건설공사_갑':
        ...
        return 0
    return result
def work_progress_chart(progress_chart, money):
    if 70 > progress_chart >= 50:
        cost = money * 0.5
    elif 90 > progress_chart >= 70:
        cost = money * 0.7
    elif progress_chart >= 90:
        cost = money * 0.9
    else:
        return 0
    return cost
def cms_2(Material_cost, labor_costs, kind):
    dsy = Material_cost+labor_costs
    if kind == '일반건설공사_갑':
        rate=0.0215
        result=dsy*rate
...
while True:
    ...
    if e == '예':
        cost_2=int(cms_2(a,b,c))
        use_money=int(work_progress_chart(d,cost_2))
        if cost_2 == 0:
        ...
        cost=int(cms(a,b,c))
        use_money=int(work_progress_chart(d,cost))
        if cost == 0:
            print('정보가 알맞지 않습니다. 정보를 다시 입력해주세요.')
        elif use_money == 0:
